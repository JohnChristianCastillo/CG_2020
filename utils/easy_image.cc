...
void img::EasyImage::draw_zbuf_line(
    ...
    //dus speciale gevallen: 1. if punt == A or 2.punt == B  hier kennen wij X al
    assert(x0 < this->width);
    assert(y0 < this->height);
    ...
    //als punten A en B hetzelfde zijn moeten wij nog altijd checken of de 1/z van A of B kleiner is in Zbuf op dat pixel
    if (x0 == x1)
    {
        //special case for x0 == x1  //this is when you need to draw a vertical line until the Y coordinate of last point
        //we still need to check if: the z of FIRST and LAST points are less than Z buffer on that pixel|||
        // if the points in between First point and Last point are good
        for (unsigned int i = std::min(y0, y1); i <= std::max(y0, y1); i++)
        {
            if(i == y0 || i == y1){                      //check if  point is less than what's stored in zBuffer;
                if(zBuffer.allowedByZBuffer(z0, x0, i)){ //if so replace that zbuffer pixel value and change pixel color
                    zBuffer[i][x0] = 1/z0;               //skip otherwise
                    (*this)(x0, i) = color;
                }
            }
            ...
            iVar-=1; //for every point we draw or discard we subtract 1 from how iVar which remembers how many more pixels to go
        }
    }
    else if (y0 == y1)
    {
        //special case for y0 == y1  //this is when you ned to draw a horizontal line until the x coor of last point
        for (unsigned int i = std::min(x0, x1); i <= std::max(x0, x1); i++)
        {
            if(i == x0 || i == x1){  //check if point is begin or end point
                if(zBuffer.allowedByZBuffer(z0, i, y0)){ //if so replace that zbuffer pixel value and change pixel color
                    zBuffer[y0][i] = 1/z0;               //skip otherwise
                    (*this)(i, y0) = color;
                }
            }
            else{ //we are now in the points in between the two end points; we need to first calculate Zi of this point
                //after calculating Zi we do the same step as what we've done in the above code
                double Zi = calculateZi(z0, z1, pixelCountA, iVar);
                if(zBuffer.allowedByZBuffer(Zi, i, y0)){
                    zBuffer[y0][i] = 1/Zi;
                    (*this)(i,y0) = color;
                }
            }
    ...
    else
    {
        ...
        if (-1.0 <= m && m <= 1.0)
        {
            for (unsigned int i = 0; i <= (x1 - x0); i++)
            {
                //check if Begin or end point
                if(x0+i == x0 && (unsigned int)round(y0+m*i) == y0){//if point == Begin
                    if(zBuffer.allowedByZBuffer(z0, x0, y0)){ //check if  point is less than what's stored in zBuffer;
                        zBuffer[y0][x0] = 1/z0;               //if so replace that zbuffer pixel value and change pixel color
                        (*this)(x0, y0) = color;             //skip otherwise
                    }
                }
                else if(x0 + i == x1 && (unsigned int)round(y0+m*i) == y1){ //if point == end
                    if(zBuffer.allowedByZBuffer(z1, x1, y1)){ //if so replace that zbuffer pixel value and change pixel color
                        zBuffer[y1][x1] = 1/z1;               //skip otherwise
                        (*this)(x1, y1) = color;
                    }
                }
        ...
        else if (m > 1.0)
        {
            ...
            {
                //check if Begin or end point
                if((unsigned int) round(x0 + (i / m)) == x0 && y0 + i == y0){//if point == Begin
                    if(zBuffer.allowedByZBuffer(z0, x0, y0)){ //check if  point is less than what's stored in zBuffer;
                        zBuffer[y0][x0] = 1/z0;               //if so replace that zbuffer pixel value and change pixel color
                        (*this)(x0, y0) = color;             //skip otherwise
                    }
                }
                else if((unsigned int) round(x0 + (i / m)) == x1 && y0 + i == y1){ //if point == end
                    if(zBuffer.allowedByZBuffer(z1, x1, y1)){ //if so replace that zbuffer pixel value and change pixel color
                        zBuffer[y1][x1] = 1/z1;               //skip otherwise
                        (*this)(x1, y1) = color;
                    }
